...
public class HookInjectorTransformer implements IClassManipulator{
	...
	private Class defineClass(ClassLoader loader, String name, byte[] data){
		try{
			return (Class) this.defineClass.invoke(loader, name, data, 0, data.length);
		}catch(Throwable e){
			e.printStackTrace();
			throw Throwables.propagate(e);
		}
	}
	private boolean hasClassLoaderLoadedClass(ClassLoader loader, String name){
		try{
			Vector<Class> loadedClasses = (Vector) classes.get(loader);
			for(Class each : loadedClasses){
	...
	public byte[] transformClass(ClassLoader loader, String className, byte[] bytes){
		...
		if(changedAnything){
			...
			//add the MethodHandle Lookup
				...
				lookupMethod.visitLdcInsn(className);
	...
	public static String proxyInternalName(String inClass, String method, String desc){
		return (proxyPackageName(inClass) 
				+ProxyListing.instance.generateClassName(inClass, method, desc).replace('.', '/'));
